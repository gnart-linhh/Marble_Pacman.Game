...
GameManager::GameManager(SDL_Renderer* &renderer) {
    ...
    egBoard = loadImage(renderer, "Assets/Images/endgame.png");
    hsBoard = loadImage(renderer, "Assets/Images/newHighscore.png");
...
void GameManager::checkScoreData(const std::vector<std::string> &scoreData) {
    for (int i = 0; i < scoreData.size(); ++i) {
        int t = 0;
        int j = 0;
        while (j < scoreData.at(i).length() && scoreData.at(i)[j] != ':') ++j;
        j += 2;
        while (j < scoreData.at(i).length()) t = t * 10 + scoreData.at(i)[j] - '0', ++j;
        if (scores > t) {
            newRecord = true;
            SDL_StartTextInput();
            pos = i;

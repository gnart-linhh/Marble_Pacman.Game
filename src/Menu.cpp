...
void Menu::init(SDL_Renderer* &renderer, const std::string imgPath, std::vector<std::string>& buttonText) {
    ...
    if (Image == nullptr)
        ...
        SDL_FreeSurface(Image);
        howToPlayTexture[0] = loadImage(renderer, "Assets/Images/HowtoPlay1.png");
        howToPlayTexture[1] = loadImage(renderer, "Assets/Images/HowtoPlay2.png");
        howToPlayTexture[2] = loadImage(renderer, "Assets/Images/HowtoPlay3.png");
        Image = IMG_Load("Assets/Images/HighScores.png");
        highScoreTexture = loadImage(renderer, "Assets/Images/HighScores.png");
        arrowTexture = loadImage(renderer, "Assets/Images/arrow-upanddown.png");
        ...
        for (int i = 0; i < TOTAL_BUTTON; ++i) {
            if (buttonText.at(i) == "Sound: ON") {
                if (Mix_Volume(-1, -1) == 0) buttonText.at(i) = "Sound: OFF";
            }
            menuButton.at(i)->loadButton(renderer, buttonText.at(i));
            menuButton.at(i)->setStatus(Button::BUTTON_OUT);
        }
...
void Menu::render(SDL_Renderer* &renderer, const std::vector<std::string> &scoreData) {
    if (currentMenuStatus == HOW_TO_PLAY) {
        ...
        for (int i = arrowHSPos; i < arrowHSPos + 7; ++i) {
            scoreText->loadRenderText(renderer, scoreData.at(i), {0, 0, 0, 255});
        ...
        for (int i = 0; i < TOTAL_BUTTON; ++i)
            menuButton.at(i)->renderButton(renderer);
    }
}
    ...
    if (e.type == SDL_KEYDOWN) {
        if (e.key.keysym.sym == SDLK_DOWN || e.key.keysym.sym == SDLK_s) {
            ...
            if (currentMenuStatus == HIGH_SCORES) {
                ...
                menuButton.at(currentButtonID)->setStatus(Button::BUTTON_OUT);
            ...
            if (currentMenuStatus == HOW_TO_PLAY) {
                ...
                menuButton.at(currentButtonID)->setStatus(Button::BUTTON_IN);
                std::string menuText = menuButton.at(currentButtonID)->getText();
                if (menuText == "New Game") currentMenuStatus = PLAY_BUTTON_PRESSED;
                    ...
                    menuButton.at(currentButtonID)->changeSoundButton(renderer);
                    Mix_Volume(-1, 0);
                }
